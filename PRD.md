...
- **Correlation Matrix Heat-map:** A full N×N grid of the pairwise Pearson matrix already produced by `/api/engine`, reachable from the `Matrix` nav entry. Correlation is *polarity* data — which side of zero — so it takes a **diverging** scale: two hues around a neutral midpoint, never a sequential ramp or a rainbow. Blue and red are used because they read as opposites and stay separable under colour-vision deficiency. Seven classes (three per arm plus the neutral) is the practical ceiling before adjacent classes blur. Each arm is lightness-monotone with visible steps, and the light and dark themes carry independently stepped values: the dark arms hold their inner steps at low chroma, because a saturated dark red at near-neutral lightness makes a weak 0.2 correlation shout as loudly as a strong one. Hovering gives a crosshair plus an exact-coefficient readout — placed below the grid rather than floating, since a card chasing the cursor would cover the neighbouring cells being compared — and a "Show values" toggle renders the numbers as a table so the reading never depends on colour alone.
